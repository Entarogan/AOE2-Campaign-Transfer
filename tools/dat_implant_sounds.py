"""
将「源」数据模组中指定区间的语音 (sounds) 植入「目标」数据模组。
...
def implant_sounds_from_dat(
    ...
    *,
    ...
    """
    将源 dat 中 [source_start, source_end] 的语音植入目标 dat。
    若 target_start 为 None，则使用与源相同的位置（覆盖）；否则植入到 target_start 起（迁移）。

    Args:
        source_dat_path: 源 dat 路径或已解析的 DatFile
        target_dat_path: 目标 dat 路径或已解析的 DatFile（将被原地修改）
        output_path: 输出路径；为 None 时不写入磁盘
        source_start: 源语音起始 ID（含），默认 850
        source_end: 源语音结束 ID（含）；None 表示到源末尾
    ...
    n_source = len(source_data.sounds)
    if source_start >= n_source:
        raise ValueError(f"源模组语音总数 {n_source}，source_start={source_start} 越界")
    ...
    if source_end is None:
        ...
        if source_end >= n_source:
            raise ValueError(f"source_end={source_end} 越界，源语音 ID 范围 [0, {n_source - 1}]")
    ...
    if migrate:
        ...
        _ensure_length(target_data.sounds, need_len, target_data.sounds[0])
        overwritten = [
            (t_start + i, getattr(target_data.sounds[t_start + i], "id", t_start + i))
        ...
        _ensure_length(target_data.sounds, need_len, target_data.sounds[0])
        overwritten = [(source_start + i, getattr(target_data.sounds[source_start + i], "id", source_start + i)) for i in range(implant_count)]

    if overwritten and warn_overwrite:
        print("[提醒] 以下目标语音槽位将被覆盖（原数据）：")
        for idx, sid in overwritten[:20]:
            print(f"  sound[{idx}] 原 id={sid}")
    ...
    for i in range(implant_count):
        ...
        tgt_idx = t_start + i
        target_data.sounds[tgt_idx] = copy.deepcopy(source_data.sounds[src_idx])
